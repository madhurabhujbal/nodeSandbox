...
function getPosition(head, number) {
    let position = 1;
    if(head == undefined) {
        return "Empty List";
    }
    ...
    let current = head;
    while(current != undefined && current.value != number) {
        current = current.next;
        position = position + 1;
    }
    if(current == undefined)
...
module.exports = {printList, addListItem, addItemInMiddle, deleteListItem, getArray, getPosition};